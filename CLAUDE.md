...
### Deliverability
Signals applied to every send (read [DELIVERABILITY.md](DELIVERABILITY.md) for the customer-facing version):
- DKIM 2048 sign (`src/services/email-sender.ts` — key cache keyed on `domainId`, TTL 10min, private key AES-256-GCM encrypted at rest).
- SPF + DMARC generated by `src/services/dns.service.ts:generateDnsRecords`. DMARC is `p=quarantine; adkim=s; aspf=s; pct=100` plus `rua=mailto:{dmarc_rua_email}` when the domain has one set.
- `List-Unsubscribe` + `List-Unsubscribe-Post: One-Click` on every send. GET + POST endpoints at `/unsubscribe/:encodedData` (`src/routes/tracking.ts`). POST returns 204 per RFC 8058.
- `Feedback-ID` header for Gmail Postmaster.
- Envelope `MAIL FROM` uses `bounces@{domain.return_path_domain || from_domain}` (`src/services/email-sender.ts`).
- Inbound MX parses RFC 3464 DSNs and RFC 5965 ARF reports at ingress (`src/services/bounce-parser.service.ts`). Permanent bounces → `addSuppression(..., "bounce")`; complaints → `addSuppression(..., "complaint")`. Both fire webhooks (`email.bounced` / `email.soft_bounced` / `email.complained`) and **skip** inbox storage. Handled in `src/smtp/inbound-server.ts` before the inbound queue enqueue.
- Per-domain send rate limit (`domains.send_rate_per_minute`) — Redis token bucket in `src/services/email.service.ts:checkDomainRateLimit`. Skipped when Redis is absent.
- `domains.dmarc_rua_email`, `domains.return_path_domain`, `domains.send_rate_per_minute` are tunable at runtime via `PATCH /v1/domains/:id` (`domain.service.ts:updateDomain`).
...
src/
  ...
  services/
    ...
    bounce-parser.service.ts    # isDsn/isFbl + parseDsn/parseFbl — RFC 3464 + RFC 5965
