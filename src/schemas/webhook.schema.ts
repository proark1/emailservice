...
const PRIVATE_PREFIXES = ["10.", "0.", "172.16.", "172.17.", "172.18.", "172.19.", "172.20.", "172.21.", "172.22.", "172.23.", "172.24.", "172.25.", "172.26.", "172.27.", "172.28.", "172.29.", "172.30.", "172.31.", "192.168.", "fc00:", "fd00:", "fe80:", "::ffff:10.", "::ffff:172.", "::ffff:192.168.", "::ffff:127.", "::ffff:0."];
...
const httpUrl = z.string().url().max(2048).refine(
  ...
  (url) => {
    try {
      ...
      if (PRIVATE_SUFFIXES.some((s) => hostname.endsWith(s))) return false;
      return true;
    } catch { return false; }
  },
