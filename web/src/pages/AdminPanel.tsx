  ...
  return (
    <div className="min-h-screen bg-gray-50 antialiased">
      ...
      <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2 py-0.5 rounded-md bg-amber-50 text-amber-600 text-[11px] font-semibold border border-amber-200">ADMIN</span>
          </div>
          <div className="flex items-center gap-3">
            <Link to="/dashboard" className="text-[13px] text-gray-500 hover:text-gray-900 transition-colors">Dashboard</Link>
            <button onClick={async () => { await logout(); navigate("/"); }} className="text-[13px] text-gray-500 hover:text-gray-900 transition-colors">Sign out</button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        {/* Tabs */}
        <div className="flex gap-1 p-1 rounded-xl bg-gray-50 border border-gray-200 inline-flex mb-8">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all ${
                tab === t.key ? "bg-gray-100 text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
            >
              {t.label}
            </button>
        ...
        {tab === "analytics" && (
          <div className="space-y-6">
            {analyticsLoading && <p className="text-gray-500 text-[13px]">Loading analytics...</p>}
            ...
            {emailTimeSeries.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Email Volume (Last 30 Days)</h2>
                <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="text-right px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Queued</th>
                          ...
                          <th className="text-right px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Failed</th>
                        </tr>
                      </thead>
                        ...
                        {(() => {
                          ...
                          return Array.from(byDate.entries()).reverse().slice(0, 30).map(([date, statuses]) => (
                            <tr key={date} className="hover:bg-gray-50">
                              <td className="px-4 py-2 text-[13px] text-gray-600 font-mono">{date}</td>
                              <td className="px-4 py-2 text-[13px] text-gray-500 text-right">{statuses.queued || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-emerald-600 text-right">{statuses.sent || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-green-600 text-right">{statuses.delivered || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-amber-600 text-right">{statuses.bounced || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-red-600 text-right">{statuses.failed || 0}</td>
                            </tr>
                          ));
                        })()}
                      </tbody>
            ...
            {eventTimeSeries.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Event Volume (Last 30 Days)</h2>
                <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(() => {
                          ...
                          return Array.from(byDate.entries()).reverse().slice(0, 30).map(([date, types]) => (
                            <tr key={date} className="hover:bg-gray-50">
                              <td className="px-4 py-2 text-[13px] text-gray-600 font-mono">{date}</td>
                              <td className="px-4 py-2 text-[13px] text-emerald-600 text-right">{types.sent || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-green-600 text-right">{types.delivered || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-blue-600 text-right">{types.opened || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-cyan-600 text-right">{types.clicked || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-amber-600 text-right">{types.bounced || 0}</td>
                              <td className="px-4 py-2 text-[13px] text-red-600 text-right">{types.complained || 0}</td>
                            </tr>
                          ));
                        })()}
                      </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {topAccounts.length > 0 && (
                <div>
                  <h2 className="text-lg font-semibold text-gray-900 mb-4">Top Accounts</h2>
                  <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-200">
                            ...
                            <th className="text-right px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Emails</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                          {topAccounts.map((a: any) => (
                            <tr key={a.account_id} className="hover:bg-gray-50">
                              <td className="px-4 py-2">
                                <p className="text-[13px] text-gray-900">{a.name}</p>
                                <p className="text-[11px] text-gray-400">{a.email}</p>
                              </td>
                              <td className="px-4 py-2 text-[13px] text-gray-600 text-right font-mono">{a.email_count.toLocaleString()}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {topDomains.map((d: any) => (
                            <tr key={d.domain_id} className="hover:bg-gray-50">
                              <td className="px-4 py-2 text-[13px] text-gray-900 font-mono">{d.name}</td>
                              <td className="px-4 py-2">
                                <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-medium border ${
                                  d.status === "verified" ? "bg-emerald-50 text-emerald-600 border-emerald-200"
                                  : d.status === "failed" ? "bg-rose-50 text-rose-600 border-rose-200"
                                  : "bg-gray-100 text-gray-600 border-gray-200"
                                }`}>{d.status}</span>
                              </td>
                              <td className="px-4 py-2 text-[13px] text-gray-600 text-right font-mono">{d.email_count.toLocaleString()}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {webhookHealth && (
                <div>
                  <h2 className="text-lg font-semibold text-gray-900 mb-4">Webhook Health</h2>
                  <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
                    <div className="flex justify-between text-[13px]">
                      ...
                      <span className="text-gray-900 font-mono">{webhookHealth.total.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-[13px]">
                      <span className="text-gray-500">Success Rate</span>
                      <span className="text-emerald-600 font-mono">{(webhookHealth.success_rate * 100).toFixed(1)}%</span>
                    </div>
                    <div className="h-px bg-gray-200" />
                    <div className="grid grid-cols-4 gap-2 text-center">
                      {[
                        { label: "Success", value: webhookHealth.success, color: "text-emerald-600" },
                        { label: "Failed", value: webhookHealth.failed, color: "text-amber-600" },
                        { label: "Exhausted", value: webhookHealth.exhausted, color: "text-red-600" },
                        { label: "Pending", value: webhookHealth.pending, color: "text-gray-500" },
                      ].map((s) => (
                        <div key={s.label}>
              ...
              {suppressionData && (
                <div>
                  <h2 className="text-lg font-semibold text-gray-900 mb-4">Suppressions</h2>
                  <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
                    <div className="flex justify-between text-[13px]">
                      <span className="text-gray-500">Total Suppressions</span>
                      <span className="text-gray-900 font-mono">{suppressionData.total.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-[13px]">
                      <span className="text-gray-500">Last 7 Days</span>
                      <span className="text-amber-600 font-mono">+{suppressionData.recent_7d}</span>
                    </div>
                    <div className="h-px bg-gray-200" />
                    <div className="grid grid-cols-4 gap-2 text-center">
                      {[
                        { label: "Bounce", value: suppressionData.by_reason.bounce || 0, color: "text-amber-600" },
                        { label: "Complaint", value: suppressionData.by_reason.complaint || 0, color: "text-red-600" },
                        { label: "Unsub", value: suppressionData.by_reason.unsubscribe || 0, color: "text-blue-600" },
                        { label: "Manual", value: suppressionData.by_reason.manual || 0, color: "text-gray-500" },
                      ].map((s) => (
                        <div key={s.label}>
                          <p className="text-[11px] text-gray-500 mb-1">{s.label}</p>
                          <p className={`text-lg font-bold ${s.color}`}>{s.value}</p>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Event</th>
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Account</th>
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Subject</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100">
                        {recentActivity.map((e: any) => {
                          const eventColors: Record<string, string> = {
                            ...
                            clicked: "text-cyan-600 bg-cyan-50 border-cyan-200",
                            bounced: "text-amber-600 bg-amber-50 border-amber-200",
                            failed: "text-red-600 bg-rose-50 border-rose-200",
                            complained: "text-rose-600 bg-rose-50 border-rose-200",
                            queued: "text-gray-600 bg-gray-100 border-gray-200",
                          };
                          return (
                            <tr key={e.id} className="hover:bg-gray-50">
                              <td className="px-4 py-2 text-[12px] text-gray-500 font-mono whitespace-nowrap">{new Date(e.created_at).toLocaleString()}</td>
                              <td className="px-4 py-2">
                                <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-medium border ${eventColors[e.type] || "text-gray-600 bg-gray-100 border-gray-200"}`}>{e.type}</span>
                              </td>
                              <td className="px-4 py-2 text-[13px] text-gray-500">{e.account_name}</td>
                              <td className="px-4 py-2 text-[13px] text-gray-600 truncate max-w-[200px]">{e.subject}</td>
                            </tr>
                          );
                        })}
                      </tbody>
            ...
            {apiKeyUsage.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-4">API Key Usage</h2>
                <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Key</th>
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Account</th>
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Last Used</th>
                          <th className="text-left px-4 py-3 text-[11px] font-medium text-gray-500 uppercase">Status</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100">
                        {apiKeyUsage.map((k: any) => (
                          <tr key={k.id} className="hover:bg-gray-50">
                            <td className="px-4 py-2">
                              <p className="text-[13px] text-gray-900">{k.name}</p>
                              <p className="text-[11px] text-gray-400 font-mono">{k.key_prefix}...</p>
                            </td>
                            <td className="px-4 py-2">
                              <p className="text-[13px] text-gray-500">{k.account_name}</p>
                            </td>
                            <td className="px-4 py-2 text-[12px] text-gray-500 font-mono">{k.last_used_at ? new Date(k.last_used_at).toLocaleString() : "Never"}</td>
                            <td className="px-4 py-2">
                              <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-medium border ${
                                k.status === "active" ? "text-emerald-600 bg-emerald-50 border-emerald-200"
                                : k.status === "revoked" ? "text-red-600 bg-rose-50 border-rose-200"
            ...
            {!analyticsLoading && !analyticsOverview && (
              <div className="text-center py-12">
                <p className="text-gray-500 text-[13px]">No analytics data available yet. Start sending emails to see metrics.</p>
              </div>
            )}
          </div>
        )}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left px-4 py-3 text-[12px] font-medium text-gray-500 uppercase tracking-wider">User</th>
                      <th className="text-left px-4 py-3 text-[12px] font-medium text-gray-500 uppercase tracking-wider">Role</th>
                      <th className="text-left px-4 py-3 text-[12px] font-medium text-gray-500 uppercase tracking-wider">Joined</th>
                      <th className="text-right px-4 py-3 text-[12px] font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {accounts.map((a) => (
                      <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-violet-100 border border-gray-200 flex items-center justify-center text-[12px] font-semibold text-violet-600">
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              ...
                              className="px-2.5 py-1.5 text-[12px] text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                            >
                            ...
                            {a.id !== user?.id && (
                              <button
                                ...
                                className="px-2.5 py-1.5 text-[12px] text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
                              >
                                Delete
                              </button>
                            )}
