    ...
    <div>
        ...
        <div className="mb-6 p-5 rounded-2xl border border-amber-200 bg-amber-50 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[13px] text-gray-500 mt-1 leading-relaxed">You need at least one verified domain before you can send emails. Add a domain in the Domains page and complete DNS verification.</p>
              <Link to="/dashboard/domains" className="inline-flex items-center gap-1 mt-3 text-[13px] font-medium text-violet-600 hover:text-violet-700 transition-colors">
                Go to Domains
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Compose Email">
        ...
        <div className="space-y-3">
          ...
          {verifiedDomains.length > 0 && <p className="text-[11px] text-gray-400">Verified domains: {verifiedDomains.map((d) => d.name).join(", ")}</p>}
        ...
        <Table headers={["Recipient", "Subject", "Status", "Date"]}>
          {items.map((e) => (
            <tr key={e.id} className="hover:bg-gray-50">
              <td className="px-4 py-3 text-gray-900 text-[13px] font-medium">{Array.isArray(e.toAddresses) ? e.toAddresses[0] : "—"}</td>
              <td className="px-4 py-3 text-gray-500 text-[13px]">{e.subject}</td>
              <td className="px-4 py-3"><Badge variant={statusVariant(e.status)}>{e.status}</Badge></td>
              <td className="px-4 py-3 text-gray-500 text-[13px]">{new Date(e.createdAt).toLocaleString()}</td>
            </tr>
          ))}
        </Table>
      )}
...
function InboxPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-0 -m-4 sm:-m-6 lg:-m-8 h-[calc(100vh)] overflow-hidden">
      {/* Email list */}
      <div className="w-full lg:w-[360px] shrink-0 border-r border-gray-200 flex flex-col h-full">
        <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-[15px] font-semibold text-gray-900">Inbox</h2>
          <span className="text-[12px] text-gray-500">{unread.length} unread</span>
        </div>
        <div className="flex-1 overflow-y-auto">
          {activeItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              ...
              <p className="text-[13px] text-gray-500">No emails yet</p>
              <p className="text-[12px] text-gray-300 mt-1">Emails sent to your verified domains will appear here</p>
            </div>
          ) : activeItems.map((email) => (
            <button
              key={email.id}
              onClick={() => openEmail(email)}
              className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition-colors ${selected?.id === email.id ? "bg-gray-100" : ""} ${!email.isRead ? "bg-white" : ""}`}
            >
              <div className="flex items-center justify-between mb-0.5">
                <span className={`text-[13px] truncate ${!email.isRead ? "font-semibold text-gray-900" : "text-gray-600"}`}>
                  {email.fromName || email.fromAddress}
                </span>
                <span className="text-[11px] text-gray-400 shrink-0 ml-2">{timeAgo(email.createdAt)}</span>
              </div>
              <div className="flex items-center gap-1.5">
                {!email.isRead && <div className="w-1.5 h-1.5 rounded-full bg-violet-500 shrink-0" />}
                ...
                <p className={`text-[12px] truncate ${!email.isRead ? "text-gray-600" : "text-gray-500"}`}>{email.subject}</p>
              </div>
              <p className="text-[11px] text-gray-300 truncate mt-0.5">{email.toAddress}</p>
            </button>
          ))}
        </div>
      </div>
          ...
          <>
            ...
            <div className="px-6 py-4 border-b border-gray-200 shrink-0">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-[16px] font-semibold text-gray-900 truncate pr-4">{selected.subject}</h2>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => toggleStar(selected.id, selected.isStarred)} className={`p-1.5 rounded-lg hover:bg-gray-100 ${selected.isStarred ? "text-amber-400" : "text-gray-400"}`}>
                  ...
                  <button onClick={() => archive(selected.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-900 hover:bg-gray-100" title="Archive">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0l-3-3m3 3l3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
                  </button>
                  <button onClick={() => deleteEmail(selected.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50" title="Delete">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                  </button>
                </div>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-100 to-indigo-100 border border-gray-200 flex items-center justify-center text-[11px] font-semibold text-violet-600">
                  {(selected.fromName || selected.fromAddress)?.charAt(0)?.toUpperCase()}
                </div>
                <div>
                  <p className="text-gray-900 font-medium">{selected.fromName || selected.fromAddress}</p>
                  <p className="text-gray-500 text-[12px]">{selected.fromAddress} &rarr; {selected.toAddress}</p>
                </div>
                <span className="text-[12px] text-gray-400 ml-auto">{new Date(selected.createdAt).toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              {selected.htmlBody ? (
                ...
                <pre className="text-[13px] text-gray-600 whitespace-pre-wrap font-sans leading-relaxed">{selected.textBody || "(empty)"}</pre>
              )}
            </div>
            ...
            <div className="shrink-0 px-6 py-3 border-t border-gray-200">
              {!replyOpen ? (
                ...
                <div className="space-y-3">
                  {error && <div className="px-3 py-2 rounded-lg bg-red-50 border border-red-200 text-red-600 text-[13px]">{error}</div>}
                  ...
                  {verifiedDomains.length > 0 && <p className="text-[11px] text-gray-400">You can send as any address on: {verifiedDomains.map((d: any) => d.name).join(", ")}</p>}
