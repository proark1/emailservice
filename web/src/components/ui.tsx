...
export function EmptyState({ title, desc, action }: { title: string; desc: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center border border-gray-200 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-900 animate-fade-in">
...
// --- Button ---
export function Button({ children, onClick, variant = "primary", disabled, type = "button", size = "md", loading }: {
  children: ReactNode; onClick?: () => void; variant?: "primary" | "secondary" | "danger"; disabled?: boolean; type?: "button" | "submit"; size?: "sm" | "md" | "lg"; loading?: boolean;
}) {
...
export function Input({ label, error, ...props }: { label: string; error?: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      ...
      <input {...props} className={`w-full h-10 px-3.5 bg-white dark:bg-gray-800 border rounded-xl text-gray-900 dark:text-gray-100 text-sm placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 transition-all ${error ? "border-red-400 ring-1 ring-red-200" : "border-gray-300 dark:border-gray-600"}`} />
...
// --- Toast ---
export function Toast({ message, variant = "error", onClose }: { message: string; variant?: "error" | "success"; onClose: () => void }) {
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[70] max-w-sm animate-slide-up">
      <div className={`flex items-center gap-3 rounded-xl border px-4 py-3 shadow-lg ${isSuccess ? "border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-900/30" : "border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/30"}`}>
        {isSuccess ? (
        ...
        <p className={`text-[13px] ${isSuccess ? "text-emerald-700 dark:text-emerald-300" : "text-red-700 dark:text-red-300"}`}>{message}</p>
        <button onClick={onClose} className={`ml-auto transition-colors ${isSuccess ? "text-emerald-400 hover:text-emerald-600" : "text-red-400 hover:text-red-600"}`}>
...
export function useToast() {
  const [state, setState] = useState<{ message: string; variant: "error" | "success" } | null>(null);
  const showError = useCallback((msg: string) => setState({ message: msg, variant: "error" }), []);
  const showSuccess = useCallback((msg: string) => setState({ message: msg, variant: "success" }), []);
  const toast = state ? <Toast message={state.message} variant={state.variant} onClose={() => setState(null)} /> : null;
